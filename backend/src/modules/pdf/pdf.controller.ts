...
export class PdfController {
  ...
  @ApiOperation({ summary: 'Generate printable PDF for Teacher Earnings & Payout Summary' })
  async downloadTeacherPayoutPdf(
    @Res() res: Response,
    @Param() query: any,
  ) {
    const month = (res.req.query.month as any) ? Number(res.req.query.month) : 7;
    const year = (res.req.query.year as any) ? Number(res.req.query.year) : 2026;

    const pdfBuffer = await this.pdfService.generateTeacherPayoutPdf(month, year);
