...
export class SingleStudentAttendanceDto {
  ...
  @ApiPropertyOptional({ example: 'Medical excuse note attached', description: 'Optional remarks' })
  @IsOptional()
  @IsString()
  remarks?: string;
}
...
export class MarkBulkAttendanceDto {
  ...
  @ApiProperty({ example: '2026-07-25', description: 'Attendance date (YYYY-MM-DD)' })
  @IsNotEmpty({ message: 'Attendance date is required.' })
  @IsDateString({}, { message: 'Invalid attendance date format.' })
  attendanceDate: string;

  @ApiProperty({ type: [SingleStudentAttendanceDto], description: 'Array of student attendance records' })
  @IsArray()
