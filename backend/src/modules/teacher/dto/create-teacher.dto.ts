...
export class CreateTeacherDto {
  ...
  @ApiProperty({ example: '951234567V', description: 'Sri Lankan NIC (old or 12-digit) or Passport' })
  @IsNotEmpty({ message: 'NIC or Passport is required.' })
  @IsString()
  nicOrPassport: string;

  @ApiProperty({ example: '1985-08-20', description: 'Date of Birth (YYYY-MM-DD)' })
  @IsNotEmpty({ message: 'Date of birth is required.' })
  @IsDateString({}, { message: 'Invalid date format for date of birth.' })
  dob: string;

  @ApiProperty({ enum: Gender, example: Gender.MALE, description: 'Teacher gender' })
  ...
  @ApiPropertyOptional({ example: 'silva@sector.lk', description: 'Email address' })
  @IsOptional()
  @IsEmail({}, { message: 'Invalid email address format.' })
  email?: string;

  @ApiPropertyOptional({ example: '45, Galle Road, Colombo 03', description: 'Home address' })
  @IsOptional()
  @IsString()
  address?: string;

  @ApiPropertyOptional({ example: '+94719876543 (Wife)', description: 'Emergency contact name and number' })
  @IsOptional()
  @IsString()
  ...
  @ApiPropertyOptional({ example: 'https://storage.sector.lk/photos/tch-001.jpg', description: 'Profile photo URL' })
  @IsOptional()
  @IsString()
  photoUrl?: string;

  // Commission Config
  @ApiProperty({ example: 70.0, description: 'Default monthly tuition fee commission % (e.g. 70%)' })
  ...
  @ApiPropertyOptional({ enum: CommissionType, example: CommissionType.PERCENTAGE, description: 'Admission fee commission calculation type' })
  @IsOptional()
  @IsEnum(CommissionType)
  admissionCommissionType?: CommissionType;

  @ApiPropertyOptional({ example: 20.0, description: 'Admission fee commission value (e.g. 20% or LKR 1000)' })
  @IsOptional()
